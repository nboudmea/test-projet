  ...
  return (
    <div className="page-container">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        {/* Statistiques avec style card-dark */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <Card className="card-dark hover:bg-slate-600/30 transition-all duration-300">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-400 mb-1">Total projets</p>
          ...
          <Card className="card-dark hover:bg-slate-600/30 transition-all duration-300">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-400 mb-1">Fiches créées</p>
          ...
          <Card className="card-dark hover:bg-slate-600/30 transition-all duration-300">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-400 mb-1">Quiz complétés</p>
                  <p className="text-3xl font-bold text-white">
        ...
        {/* Barre d'outils avec style glass */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center space-x-4 flex-1">
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                ...
                className="pl-10 card-dark text-white placeholder-slate-400 focus:border-memento-teal"
              />
            </div>
            <Button variant="outline" size="sm" className="border-slate-600 card-dark text-slate-300 hover:bg-slate-600/50 hover:text-white">
          ...
          <div className="flex items-center space-x-2">
            <div className="flex card-dark rounded-lg p-1 border border-slate-600">
              <Button
                variant={viewMode === 'grid' ? 'default' : 'ghost'}
                size="sm"
                onClick={() => setViewMode('grid')}
                className={viewMode === 'grid' ? 'bg-memento-teal text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}
              >
              ...
              <Button
                ...
                className={viewMode === 'list' ? 'bg-memento-teal text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}
              >
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
              ...
              <DialogContent className="card-dark border-slate-600 text-white">
                <DialogHeader>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex justify-end space-x-3">
                    <Button variant="outline" onClick={() => setIsCreateDialogOpen(false)} className="border-slate-600 text-slate-300 hover:bg-slate-700/50">
                      Annuler
                    </Button>
        ...
        {/* Grille de projets avec style cohérent */}
        {filteredProjects.length === 0 ? (
          <Card className="border-2 border-dashed border-slate-600 card-dark">
          ...
          <div className={viewMode === 'grid' 
            ...
            {filteredProjects.map((project) => (
              <Card 
                key={project.id} 
                className="card-dark hover:bg-slate-600/40 hover:border-slate-500/50 transition-all duration-300 group cursor-pointer"
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400">Fiches: <span className="text-white font-medium">{project.flashcards.length}</span></span>
                      <span className="text-slate-400">Quiz: <span className="text-white font-medium">{project.quizzes.length}</span></span>
                    </div>
                    ...
                    <Button
                      ...
                      className="w-full bg-slate-700/50 hover:bg-memento-teal text-white transition-colors"
                    >
                      Ouvrir le projet
                    </Button>
                  </div>
